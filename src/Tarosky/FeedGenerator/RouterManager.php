...
class RouterManager {
	...
	public function rewrite_rules( $rules ) {
		...
		$service_identifiers = [];
		foreach ( $dm->get_services() as $service_name => $service_id ) :
	 ...
	 * @return boolean
	 */
	public function is_feed( $wp_query ) {
		$is_feed = false;
		$service_identifiers = [];
		...
		if ( $wp_query->is_feed && $wp_query->get( self::PARAM_NAME_ID ) && in_array( $wp_query->get( self::PARAM_NAME_ID ), $service_identifiers ) ) {
			$is_feed = true;
		}
	 ...
	 * @return string
	 */
	public function get_feed_id( $wp_query ) {
		$ret_id = '';
